...
docker_network_exists=$(podman network ls | grep $docker_network_name | awk '{print $2}')
if [ -n "$docker_network_exists" ]; then
    echo "容器网络 $docker_network_name 已存在"
    #if $docker_network_name's driver != bridge exit
    docker_network_driver=$(podman network inspect $docker_network_name | grep Driver | awk '{print $2}' | grep bridge)
    ...
    podman network create $docker_network_name -d bridge --ipv6 --subnet=$the_subnet --gateway=$the_gateway
