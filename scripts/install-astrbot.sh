#!/bin/bash
# podman run -itd -p 6185:6185 -p 6199:6199 -v $PWD/data:/AstrBot/data -v /etc/localtime:/etc/localtime:ro -v /etc/timezone:/etc/timezone:ro --name astrbot m.daocloud.io/docker.io/soulter/astrbot:latest
domain=$1
base_data_dir=$2
...
case $install_shipyard in
    [Yy]* ) 
        ...
        port=8114
        docker_group_id=$(cat /etc/group | grep -e podman | cut -d ":" -f 3)
