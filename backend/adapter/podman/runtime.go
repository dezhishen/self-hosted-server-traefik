...
func (r *Runtime) podman(args ...string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("podman %s: %s: %w", strings.Join(args, " "), strings.TrimSpace(string(out)), err)
	}
...
func (r *Runtime) ContainerInspect(containerID string) (*contracts.ContainerInfo, error) {
	out, err := r.podman("inspect", containerID)
	if err != nil {
		return nil, err
	}
...
func (r *Runtime) PullImage(image string) error {
	_, err := r.podman("pull", image)
	return err
}

func (r *Runtime) ImageList() ([]contracts.ImageInfo, error) {
	out, err := r.podman("images", "--format", "{{json .}}")
	if err != nil {
		return nil, err
	}
	lines := strings.Split(out, "\n")
...
func (r *Runtime) NetworkRemove(networkID string) error {
	_, err := r.podman("network", "rm", networkID)
	return err
}

func (r *Runtime) NetworkInspect(networkID string) (*contracts.NetworkInfo, error) {
	out, err := r.podman("network", "inspect", networkID)
	if err != nil {
		return nil, err
	}
...
func (r *Runtime) NetworkList() ([]contracts.NetworkInfo, error) {
	out, err := r.podman("network", "ls", "--format", "{{json .}}")
	if err != nil {
		return nil, err
	}
	lines := strings.Split(out, "\n")
...
func (r *Runtime) NetworkConnect(networkID string, containerID string) error {
	_, err := r.podman("network", "connect", networkID, containerID)
	return err
}
...
func (r *Runtime) VolumeInspect(volumeID string) (*contracts.VolumeInfo, error) {
	out, err := r.podman("volume", "inspect", volumeID)
	if err != nil {
		return nil, err
	}
...
func (r *Runtime) VolumeList() ([]contracts.VolumeInfo, error) {
	out, err := r.podman("volume", "ls", "--format", "{{json .}}")
	if err != nil {
		return nil, err
	}
	lines := strings.Split(out, "\n")
...
func (r *Runtime) Info() (*contracts.RuntimeInfo, error) {
	out, err := r.podman("info", "--format", "{{json .}}")
	if err != nil {
		return nil, err
	}
	var info struct {
		ServerVersion string `json:"serverVersion"`
		OperatingSystem string `json:"host.os"`
		Architecture   string `json:"host.arch"`
